...
依次遍历div,并从中获取需要的内容
'''
def read_div(div_list,group_number,is_group,group_name,group_user_number,i,url,Massage_id,member_list,user_detial_id,user_relation_id,user_all_list,relation_all_list,group_list,flag,types,time2,number,start_time,information_path,message_count,information_list,group_relation_list,group_detail_list,user_detail_list,div_flag,last_message_count,serviceid,save_path,div_count,div_count1):
    ...
    if(div_flag==False):
        ...
        print(len(div_list))
        for div in div_list[div_count1-1:]:
            if(len(div_list)!=div_count):
                div_count1 = div_count1+1
            div_flag =True
            # 这里每一天的聊天记录是一个div，class为position-relative
            #soup = BeautifulSoup(div.get_attribute('innerHTML'),'html.parser')
            ...
            for content_main in chat_content[last_message_count-1:]:
                last_message_count = last_message_count+1
                flag[group_number] = int(flag[group_number])+1 
    ...
    return Massage_id,user_detial_id,user_relation_id,user_all_list,relation_all_list,group_list,flag,message_count,start_time,information_list,group_relation_list,group_detail_list,user_detail_list,last_message_count,div_flag,div_count,div_count1
    ...
    while (1):
        for i in range(1,len(group_user_number)+1):
            ...
            if(group_id==group_number):
                ...
                #print(len(div_list))
                Massage_id,user_detial_id,user_relation_id,user_all_list,relation_all_list,group_list,flag,message_count,start_time, information_list,group_relation_list,group_detail_list,user_detail_list,last_message_count,div_flag,div_count,div_count1 = read_div(div_list,group_number,is_group,group_name,group_user_number,i,url1,Massage_id,member_list,user_detial_id,user_relation_id,user_all_list,relation_all_list,group_list,flag,types,time2,number,start_time,information_path,message_count,information_list,group_relation_list,group_detail_list,user_detail_list,div_flag,last_message_count,serviceid,save_path,div_count,div_count1)
