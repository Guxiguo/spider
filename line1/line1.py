...
def get_text_img_voice(content_text_div,content_img,voices,filename1,file_size,timestamp1,save_path):
    ...
    filename111 = save_path+'image\\'+timestamp1
    # 获取具体的聊天记录
    if content_text_div != []:
        
        if(len(content_text_div)==1):
            content_list.append('text: '+content_text_div[0].get_attribute('textContent').strip())
        else:
            ...
            content_list.append('text: '+str1111)
        ...
        for content in content_img:
            ...
            if class_img == 'chat-item-sticker' or class_img == 'sticon emojione':
                ...
                if(url[-3:] in ['png','jpg']):
                    ...
                    filename1 = save_path+'image/'+timestamp1+'/'+timestamp+'.'+ url[-3:]
                    try:
    ...
    while (1):
        for i in range(1,len(group_user_number)+1):
            ...
            if(group_id==group_number):
                ...
                timestamp = str(int(time.time()))
                os.makedirs(save_path+'group/'+timestamp)
                os.makedirs(save_path+'group_info/'+timestamp)
                os.makedirs(save_path+'relation/'+timestamp)
                os.makedirs(save_path+'user/'+timestamp)
                os.makedirs(save_path+'image/'+timestamp)
                group_detial_path = save_path+'group/'+timestamp+'/'+group_id+'_'+timestamp+'_group.json'
                group_relation_path = save_path+'relation/'+timestamp+'/'+group_id+'_'+timestamp+'_relation.json'
                group_info_path = save_path+'group_info/'+timestamp+'/'+group_id+'_'+timestamp+'_info.json'
                user_detail_path = save_path+'user/'+timestamp+'/'+group_id+'_'+timestamp+'_user.json'
