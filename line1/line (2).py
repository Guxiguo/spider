...
def get_text_img_voice(content_text_div,content_img,voices,filename1,file_size,timestamp1):
    content_list = []
    image_info = {}
    voice_info = {}
    # 获取具体的聊天记录
        ...
        for content in content_img:
            ...
            if class_img == 'chat-item-sticker' or class_img == 'sticon emojione':
                ...
                if(url[-3:] in ['png','jpg']):
                    ...
                    filename1 = 'line/'+timestamp1+'/image/'+timestamp+'.'+ url[-3:]

                    image_size = dowenload_image(url,filename1)
                    image_info['path'] = filename1
                    image_info['length'] = str(int(image_size))
                    image_info['download_datetime'] = str(datetime.now().replace(microsecond=0))
                else:
                    image_info['type'] = 'mp4'
                    image_info['url'] = url
                    ...
                    image_size = dowenload_image(url,filename1)'''
                    image_info['path'] = filename1
                    image_info['length'] = str(file_size)
                    image_info['download_datetime'] = str(datetime.now().replace(microsecond=0))
    ...
    if voices != []:
        for voice in voices:
            ...
            voice_info['type'] = 'm4a' 
            voice_info['path'] = filename1
            voice_info['time'] = voice.text.strip()
            voice_info['length'] = file_size
            voice_info['download_datetime'] = str(datetime.now().replace(microsecond=0))

    return content_list, image_info, voice_info

def user_detail(user_detail_file,member_list,user_detial_id,user_all_list,user_detail_list):
    for member in member_list:
        ...
        if user_is_excist(user_all_list,member) == False:
            ...
            user_all_list.append(member)
            user_detail_list[user_detial_id] = user_detail
            #json.dump(user_detail,user_detail_file,ensure_ascii=False)
    return user_detial_id,user_all_list,user_detail_list



def group_detail(group_detial_file,i,group_number,group_name,group_user_number,url,group_detail_list):
    group_detail = {}
    ...
    group_detail_list[i] = group_detail
    #json.dump(group_detail,group_detial_file,ensure_ascii=False) 
    return group_detail_list

def group_relation(group_relation_file,user_relation_id,group_number,member_list,first_time,last_time,relation_all_list,group_relation_list):
    ...
    for member in member_list:
        ...
        if group_relation_is_excist(relation_all_list,group_number,member) == False:
            ...
            group_relation_list[user_relation_id] = group_relation
...
def group_info(group_info_file,content_list,date,Massage_id,group_number,sender_type,true_time,data_id_list,user_name,types,time2,number,start_time,information_path,message_count,information_list,image_info,voice_info):
    if len(data_id_list) == 1:
        ...
        if (bool(image_info)):
            if(next(iter(image_info.values()))=='mp4'):
        ...
        #messages['pubdate'] = date
        messages['pubtime'] = str(formatted_date)+" "+true_time
        ...
        information_list[Massage_id]=messages
        ...
        for data in data_id_list:
            ...
            formatted_date = date1.strftime("%m-%d")
            #messages['pubdate'] = date
            messages['pubtime'] = formatted_date+" "+true_time
            messages['data_source'] = 'TRS'
            messages['update_time'] = str(datetime.now().replace(microsecond=0))
            messages['crawler_time'] = str(datetime.now().replace(microsecond=0))
            messages['intime'] = str(datetime.now().replace(microsecond=0))
...
def read_div(div_list,group_number,is_group,group_name,group_user_number,i,url,Massage_id,member_list,user_detail_file,group_detial_file,group_relation_file,group_info_file,user_detial_id,user_relation_id,user_all_list,relation_all_list,group_list,flag,types,time2,number,start_time,information_path,message_count,information_list,group_relation_list,group_detail_list,user_detail_list,timestamp):
    ...
    last_time = init_last_time(member_list)
     # 判断用户是否已经存在，若不存在才存储
    user_detial_id,user_all_list,user_detail_list = user_detail(user_detail_file,member_list,user_detial_id,user_all_list,user_detail_list)
    if group_number not in group_list:
        group_list.append(group_number)
        flag = init_flag(flag,group_number)
        group_detail_list = group_detail(group_detial_file,i,group_number,group_name,group_user_number,url,group_detail_list)
    #sleep_time()
    ...
    for div in div_list:
        ...
        for content_main in chat_content[start_index:]:
            ...
            content_list,image_info,voice_info = get_text_img_voice(content_text_div,content_img,voices,filename,file_size,timestamp)
            # 记录消息ID
            data_id_list = []
            ...
            Massage_id,message_count,start_time,information_list = group_info(group_info_file,content_list,date,Massage_id,group_number,sender_type,true_time,data_id_list,user_name,types,time2,number,start_time,information_path,message_count,information_list,image_info,voice_info)
            
   
    # 判断用户关系是否存在，若不存在则存储
    user_relation_id,relation_all_list,group_relation_list = group_relation(group_relation_file,user_relation_id,group_number,member_list,first_time,last_time,relation_all_list,group_relation_list)   
    
    return Massage_id,user_detial_id,user_relation_id,user_all_list,relation_all_list,group_list,flag,message_count,start_time,information_list,group_relation_list,group_detail_list,user_detail_list
    ...
    while (1):
        for i in range(1,len(group_user_number)+1):
            ...
            if(group_id==group_number):
                timestamp = str(int(time.time()))
                os.makedirs(save_path+timestamp)
                os.makedirs(save_path+timestamp+'/image')
                group_detial_path = save_path+timestamp+'/'+group_id+'_'+timestamp+'_group.json'
                group_relation_path = save_path+timestamp+'/'+group_id+'_'+timestamp+'_relation.json'
                group_info_path = save_path+timestamp+'/'+group_id+'_'+timestamp+'_info.json'
                user_detail_path = save_path+timestamp+'/'+group_id+'_'+timestamp+'_user.json'
                information_list1 = []
                group_relation_list1 = []
                user_detail_list1 = []
                group_detail_list1 = []
                ...
                information_list = {}
                information_list['severiceid'] = severiceid
                group_relation_list = {}
                group_relation_list['severiceid'] = severiceid
                group_detail_list = {}
                group_detail_list['severiceid'] = severiceid
                user_detail_list ={}
                user_detail_list['severiceid'] = severiceid
                ...
                json.dump(user_detail_list1,user_detail_file,ensure_ascii=False)
                state = 'run'
                save_information(information_path,start_time,state,Massage_id,user_detial_id,user_relation_id,i)
                time.sleep(int(number))
