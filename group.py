...
proxies = {"http": "http://127.0.0.1:7890"}  # WARNING: 修改代理链接
# 命令行参数 =  end  =
...
def run_task(driver: webdriver):
    """
    执行任务并根据任务文件中的状态来控制任务的执行流程。

    Args:
        driver (webdriver): 用于执行网页操作的 Selenium WebDriver 实例。

    功能说明：
    1. 从指定路径读取任务文件 (task.json)。
    2. 遍历任务文件中的所有任务。
    3. 如果找到类型为 'mewe' 的任务，并且任务 URL 与输入的 URL 相匹配，且任务状态为 '-1'：
        a. 将任务状态更新为 '2'。
        b. 将更新后的任务列表写回文件。
        c. 打印退出信息，并退出浏览器和程序。
    """
    # 从命令行参数中获取任务文件路径，并读取任务文件
    task_file_path = f'{sys.argv[7]}/task.json'
    with open(task_file_path, 'r') as task_file:
        task = json.loads(task_file.read())

    # 遍历任务，查找特定条件的任务并执行相应操作
    for i in task:
        # 检查任务类型、URL 和在线状态
        if i['task_type'] == 'mewe' and i['task_url'] == sys.argv[6] and i['online'] == '-1':
            # 更新任务状态为 '2'，表示任务正在执行
            i['online'] = '2'
...
def save_data():
    ...
    global user
    # 检查文件夹是否已存在
    messages_folder_path = f"{save_path}/{title}/messages"
    comments_folder_path = f"{save_path}/{title}/comments"
    user_folder_path = f"{save_path}/{title}/user"
    if not os.path.exists(f"{save_path}/{title}"):
        os.makedirs(f"{save_path}/{title}")
...
def save_img(imgUrl):
    ...
    try:
        ...
        if imgUrl not in allimage:
            imgDict = {"type": "jpg", "url": imgUrl, "path": "", "name": "", "length": 0, "download_datetime": strftime("%Y-%m-%d %H:%M:%S", localtime())}
...
def copy(user_data_dir1):
    ...
    if os.path.exists(user_data_dir1) is False:
...
driver = ""
path = argv["browser_data"]
# driver = load_driver(argv["group"], argv["browser"], path)
...
WebDriverWait(driver, 5, 0.5).until(EC.presence_of_element_located((By.ID, "email"))).send_keys(email)
WebDriverWait(driver, 5, 0.5).until(EC.presence_of_element_located((By.ID, "password"))).send_keys(password)
...
while True:
    ...
    for div in sonDiv_eles[::-1]:
        tmp += 1
        # state['messages'] += 1
        oneMessage = {"serviceid": argv["serviceid"], "user_url": "", "md5": "", "post_id": "", "pubtime": "", "content": "", "image_info": [], "share": 0, "emoji": 0, "crawler_time": strftime("%Y-%m-%d %H:%M:%S", localtime()), "user_name": ""}
        try:  # 消息-try
        ...
        if oneLoopUserUrl:
            ...
            for iuser in oneLoopUserUrl:
                try:  # 用户2-try
                    ...
                    sleep(3)
                    # 在新窗口中爬取数据
                    mainProfile = driver.find_elements(By.XPATH, '//*[@id="ember21"]')  # WARNING: 用户标签-每次迭代需修改
