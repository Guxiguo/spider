...
user_data_dir = os.path.join("C:\\Users\\ISAT\\AppData\\Local\\Microsoft\\Edge\\User Data",script_id)

def copy(user_data_dir1):
    src_dir = "C:\\Users\\ISAT\\AppData\\Local\\Microsoft\\Edge\\User Data\\Default"
    os.chmod(src_dir,0o77)
    if os.path.exists(user_data_dir1)==False:
        os.makedirs(user_data_dir1)
    os.chmod(user_data_dir1,0o77)
    try:
        shutil.copytree(src_dir,user_data_dir1,dirs_exist_ok=True)
    except Exception:
        pass
...
def get_div(driver,url):
    ...
    # 定位到你想要滚动的元素
    # 请将"your_selector"替换为你的元素的CSS选择器
    element = element = driver.find_element_by_xpath(scroll)

    # 获取初始滚动条位置
    initial_scroll_position = driver.execute_script("return arguments[0].scrollTop;", element)
...
def get_text_img_voice(content_text_div,content_img,voices,filename1,file_size,save_path):
    content_list = []
    image_info = []
    voice_info = []
...
def user_detail(member_list,user_detial_id,user_all_list,user_detail_list,serviceid):
...
def group_relation(user_relation_id,group_number,member_list,first_time,last_time,relation_all_list,group_relation_list,serviceid):
...
def group_info(content_list,date,Massage_id,group_number,sender_type,true_time,data_id_list,user_name,types,time2,number,start_time,information_path,message_count,information_list,image_info,voice_info,serviceid):
...
依次遍历div,并从中获取需要的内容
'''
def read_div(div_list,group_number,group_name,group_user_number,i,url,Massage_id,member_list,user_detial_id,user_relation_id,user_all_list,relation_all_list,group_list,flag,types,time2,number,start_time,information_path,message_count,information_list,group_relation_list,group_detail_list,user_detail_list,serviceid,save_path,is_first_spider_flag,spider_div,spider_last_message_count,spider_div_count):
    #group_all_information = {}  # json中存入的字典
    first_time = init_first_time(member_list)
    last_time = init_last_time(member_list)
     # 判断用户是否已经存在，若不存在才存储
    user_detial_id,user_all_list,user_detail_list = user_detail(member_list,user_detial_id,user_all_list,user_detail_list,serviceid)
    if group_number not in group_list:
        group_list.append(group_number)
        flag = init_flag(flag,group_number)
        group_detail_list = group_detail(i,group_number,group_name,group_user_number,url,group_detail_list,serviceid)
    #sleep_time()
    #start_index = int(flag[group_number])
    #print(div_list)
    if(spider_div_count[group_number]<len(div_list)):
        spider_div_count[group_number]=len(div_list)
        spider_last_message_count[group_number] = 0
        #last_message_count=0
    # 判断用户关系是否存在，若不存在则存储
    user_relation_id,relation_all_list,group_relation_list = group_relation(user_relation_id,group_number,member_list,first_time,last_time,relation_all_list,group_relation_list,serviceid)
    if(is_first_spider_flag[group_number]==False):
        for div in div_list:
            #div_count1 = div_count1+1
            spider_div[group_number] = spider_div[group_number]+1
            #div_flag =True
            is_first_spider_flag[group_number] = True
            spider_last_message_count[group_number] = 0
            #last_message_count=0
            ...
            for content_main in chat_content:
                ...
                spider_last_message_count[group_number] = spider_last_message_count[group_number]+1
                ...
                Massage_id,message_count,start_time,information_list = group_info(content_list,date,Massage_id,group_number,sender_type,true_time,data_id_list,user_name,types,time2,number,start_time,information_path,message_count,information_list,image_info,voice_info,serviceid)
                message_count,start_time,information_list,group_relation_list,group_detail_list,user_detail_list=judeg(types,time2,number,start_time,Massage_id,information_path,message_count,save_path,group_number,information_list,group_relation_list,group_detail_list,user_detail_list)
                #print(information_list,group_relation_list,group_detail_list,user_detail_list)
    elif(is_first_spider_flag[group_number]==True):  
        #print(len(div_list))
        for div in div_list[spider_div[group_number]-1:]:
            if(len(div_list)!=spider_div_count[group_number]):
                #div_count1 = div_count1+1
                spider_div[group_number] = spider_div[group_number]+1
            #div_flag =True
            is_first_spider_flag[group_number] = True
            ...
            #print(Massage_id)
            # 遍历消息主体
            #print(len(chat_content))
            for content_main in chat_content[spider_last_message_count[group_number]:]:
                spider_last_message_count[group_number] = spider_last_message_count[group_number]+1
                #last_message_count = last_message_count+1
                flag[group_number] = int(flag[group_number])+1 
    ...
    return Massage_id,user_detial_id,user_relation_id,user_all_list,relation_all_list,group_list,flag,message_count,start_time,information_list,group_relation_list,group_detail_list,user_detail_list,is_first_spider_flag,spider_div,spider_last_message_count,spider_div_count
...
def judeg(types,time2,number,start_time,message_id,information_path,message_count,save_path,group_id,information_list,group_relation_list,group_detail_list,user_detail_list):
...
def find_and_kill_process(process_name,script_name,arg):
    ...
    for proc in psutil.process_iter(['pid','name','cmdline']):
        ...
        if process_name.lower() in proc.info['name'].lower():
            ...
            if arg in proc.info['cmdline']:
                while(1):
                    try:
                        #print(proc.info['name'],signal.SIGTERM)
...
def main1():
    ...
    while (1):
        for i in range(1,len(group_user_number)+1):
            ...
            if(group_number not in is_first_spider_flag):
                is_first_spider_flag[group_number] = False
                spider_div[group_number] = 0
                spider_last_message_count[group_number] = 0
                spider_div_count[group_number] = 0
            if(group_id==group_number):
                ...
                #print(len(div_list))
                Massage_id,user_detial_id,user_relation_id,user_all_list,relation_all_list,group_list,flag,message_count,start_time, information_list,group_relation_list,group_detail_list,user_detail_list,is_first_spider_flag,spider_div,spider_last_message_count,spider_div_count = read_div(div_list,group_number,group_name,group_user_number,i,url1,Massage_id,member_list,user_detial_id,user_relation_id,user_all_list,relation_all_list,group_list,flag,types,time2,number,start_time,information_path,message_count,information_list,group_relation_list,group_detail_list,user_detail_list,serviceid,save_path,is_first_spider_flag,spider_div,spider_last_message_count,spider_div_count)
                if information_list or group_relation_list or group_detail_list or user_detail_list:
                    timestamp = str(int(time.time()))
                    if os.path.exists(save_path+'group') ==False:
                        os.makedirs(save_path+'group')
                    if os.path.exists(save_path+'group_info') ==False:
